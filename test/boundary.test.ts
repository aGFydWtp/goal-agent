...
describe("基盤境界 6.1: Discord 署名検証・コマンドルーティングを含まない", () => {
  ...
  // コマンドルーティング・UX 文言は discord-gateway の責務。基盤の「自レイヤ」に混入して
  // はならない。
  //
  // スコープ(discord-gateway 承認後の再調整): 本検査の真の意図は「基盤自身のレイヤ
  // (types/persistence/llm/agents + env.ts)へ Discord 責務が混入する回帰を防ぐ」こと。
  // discord-gateway は同一パッケージの `src/discord/` に Discord I/O を所有し、統合点
  // `src/index.ts` に interactions パスを追加することが design で承認済みである。したがって
  //  - `src/discord/`(discord-gateway 所有)と統合点 `src/index.ts` は本検査の対象外とする。
  //  - 共有 `package.json` の `discord-interactions`(verifyKey runtime)/`discord-api-types`
  //    (型のみ)は discord-gateway が正当に追加するため許容する。
  //  - full `discord.js` / `@discordjs/*` / `tweetnacl`(自前署名実装)は引き続き禁止
  //    (design §Technology Stack: full discord.js 不使用・自前 Ed25519 不採用)。

  /** discord-gateway 所有のディレクトリ/統合点を除いた、基盤自レイヤの .ts ファイル。 */
  function foundationOwnedSrcFiles(): string[] {
    const discordPrefix = join(SRC, "discord") + "/";
    const entry = join(SRC, "index.ts");
    return collectSrcTsFiles().filter(
      (file) => !file.startsWith(discordPrefix) && file !== entry,
    );
  }

  it("package.json の依存に full discord.js/自前署名ライブラリを含まない", () => {
    ...
    // discord-gateway が正当に使う薄い依存(discord-interactions / discord-api-types)は
    // 除外。重量級スタックと自前署名ライブラリのみを禁止対象とする。
    const forbidden = ["discord.js", "tweetnacl", "@discordjs/rest", "@discordjs/core"];
  ...
  it("基盤自レイヤに Discord 署名検証(verifyKey/Ed25519/interaction 検証)の実コードを含まない", () => {
    // 実コードのみ(コメント除去後)を検査する。src/discord/ と src/index.ts は対象外。
    ...
    for (const file of foundationOwnedSrcFiles()) {
  ...
  it("src/ にコマンドルーティング層(handlers/)を含まない", () => {
    // src/discord/ は discord-gateway 所有のため存在を許容。下流の機能別コマンド
    // ハンドラ層(handlers/)は引き続き本パッケージ外。
    const dirs = srcDirNames();
